...
class ExtractionRequest(BaseModel):
    ...
    selected_articles: List[Dict[str, Any]]  # expects at least 'title' and one of 'url'/'source_url'/'link'
...
async def step2_analyze_relevance(
    ...
    """
    try:
        logger.info(f"Step 2: Analyzing {len(request.articles)} articles for UPSC relevance")
        start_time = time.time()
        ...
        for article in request.articles:
            ...
            # Process through centralized LLM service with direct Gemini fallback
            ...
            if llm_response.success and llm_response.data:
                ...
                logger.info("🔧 LLM service failed, using direct Gemini fallback for UPSC analysis")
                ai_analysis = await direct_gemini_service.analyze_upsc_relevance(
                    article.get('title', ''), 
                    article.get('content', '')
                )
                relevance_score = ai_analysis.get('upsc_relevance', 0)
                model_used = "direct-gemini-2.5-flash"
...
async def step3_extract_content(
    ...
    try:
        ...
        logger.info(f"Step 3: Extracting full content from {len(selected_articles)} articles")
        start_time = time.time()
        ...
        return {
            ...
            "data": {
                ...
                "extraction_time": extraction_time,
                "success_rate": (len(extracted_articles) / len(selected_articles) * 100) if selected_articles else 0
            },
        ...
        for article in request.articles:
            ...
            if content:
                ...
                if llm_response.success and llm_response.data:
                    ...
                    # Fallback to direct Gemini
                    logger.info("🔧 LLM service failed, using direct Gemini fallback for content refinement")
                    ai_refinement = await direct_gemini_service.refine_content(content)
                    refinement_model = "direct-gemini-2.5-flash"
                    processing_time = 0
...
async def step5_save_to_database(
    ...
    """
    try:
        logger.info(f"Step 5: Saving {len(request.processed_articles)} articles to database")
        start_time = time.time()
        ...
        for article in request.processed_articles:
            try:
                ...
                if content_hash:
                    existing = db.client.table("current_affairs").select("id").eq(
...
async def complete_pipeline(
    ...
    try:
        ...
        extraction_request_data = {
            "selected_articles": [{"title": a["title"], "url": a["source_url"]} for a in top_articles]
        }
...
def prepare_article_for_database(article: Dict[str, Any]) -> Dict[str, Any]:
    ...
    published_date = extracted_content.get('publish_date', datetime.utcnow().isoformat())
    try:
    ...
    return {
        'title': (
            ai_refinement.get('generated_title') or          # AI-generated title (priority 1)
            ai_refinement.get('enhanced_title') or           # Alternative AI title field
            extracted_content.get('title', '') or           # Fallback to extracted
            article.get('original_title', '') or            # RSS title fallback
            'Current Affairs Update'                         # Final fallback
        ),
        'content': ai_refinement.get('enhanced_content', content_text) if ai_refinement.get('enhanced_content') else content_text,
        'summary': ai_refinement.get('brief_summary', ''),
        'source': extracted_content.get('author', 'RSS Feed'),
        'source_url': article.get('url', ''),
        'published_at': extracted_content.get('publish_date', datetime.utcnow().isoformat()),
        'date': date_field,  # Add required date field
        'upsc_relevance': ai_analysis.get('upsc_relevance', 50),
        'category': map_to_proper_category(ai_analysis.get('key_topics', ['general'])[0] if ai_analysis.get('key_topics') else 'general'),
