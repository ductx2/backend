        ...
        try:
            ...
            try:
                ...
                if response.success and response.data:
                    ...
                    ai_results = response.data if isinstance(response.data, dict) else {}
                    logger.info(f"✅ AI analysis completed using {response.provider_used} for {len(ai_results)} articles")
                else:
            ...
            for idx, article in enumerate(batch):
                ...
                processed_article = ProcessedArticle(
                    ...
                    upsc_relevance=ai_analysis.get('upsc_relevance', fallback_relevance),
...
Return only the JSON response, no other text."""
            ...
            for idx, article in enumerate(articles):
                try:
                    ...
                    if article.content_hash not in existing_hashes:
                        ...
                        article_data = {
                            'title': article.title[:500] if article.title else "Untitled",  # Prevent title too long
                            'content': article.content if article.content else "",
                            'summary': article.summary[:1000] if article.summary else "",  # Prevent summary too long
                            ...
                            'upsc_relevance': max(0, min(100, int(article.upsc_relevance))),  # Ensure valid range
                            'category': article.category[:50] if article.category else "general",
                            'tags': article.tags if isinstance(article.tags, list) else [],  # Ensure array
            ...
            if articles_for_db:
                ...
                try:
                    ...
                    result = await asyncio.to_thread(
                        self.db.client.table("current_affairs").insert(articles_for_db).execute
                    )
    ...
    async def _fallback_individual_insert(self, articles_for_db: List[Dict[str, Any]]) -> int:
        ...
        for idx, article_data in enumerate(articles_for_db):
            try:
                result = await asyncio.to_thread(
                    self.db.client.table("current_affairs").insert([article_data]).execute
                )
    ...
    async def process_all_sources(self) -> Dict[str, Any]:
        ...
        final_stats = {
            ...
            'stats': {
                ...
                'avg_processing_time': total_time / len(processed_articles) if processed_articles else 0
            },
            'performance_metrics': {
